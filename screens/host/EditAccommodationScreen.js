...
// Services
import { updateAccommodation, uploadImage } from "../../services/supabase"
...
const EditAccommodationScreen = ({ route, navigation }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    price_per_night: accommodation.price_per_night ? accommodation.price_per_night.toString() : "",
    has_bike_storage: accommodation.has_bike_storage !== undefined ? accommodation.has_bike_storage : true,
    amenities: accommodation.amenities || "",
    max_guests: accommodation.max_guests ? accommodation.max_guests.toString() : "",
    bedrooms: accommodation.bedrooms ? accommodation.bedrooms.toString() : "",
    bathrooms: accommodation.bathrooms ? accommodation.bathrooms.toString() : "",
  })
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const imageUrls = [...(accommodation.images || [])]

      for (const image of images) {
        // Only upload if it's a new image (not a URL)
      ...
      // Update accommodation with image URLs
      ...
      const { data, error } = await updateAccommodation(accommodation.id, accommodationData)

      if (error) throw error

      Alert.alert(t("success"), t("accommodation_updated"), [{ text: "OK", onPress: () => navigation.goBack() }])
    } catch (error) {
      console.error("Error updating accommodation:", error)
      Alert.alert(t("error"), t("failed_to_update_accommodation"))
    } finally {
      setIsLoading(false)
    }
  }
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>{t("edit_accommodation")}</Text>
        <View style={{ width: 24 }} />
      </View>
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        <View style={styles.formContainer}>
          ...
          <Button
            title={isLoading ? t("updating") : t("update_accommodation")}
